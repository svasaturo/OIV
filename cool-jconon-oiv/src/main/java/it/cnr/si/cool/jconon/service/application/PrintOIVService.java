...
//    
    ...
    private void getRecordCSV(Session session, Folder applicationObject, Document oivObject, int applicationNumber, CMISUser user, HSSFSheet sheet, String lastRuolo, String lastDatoreLavoro, int index, boolean all) {
        ...
        LocalDateTime dataModifica = Optional.ofNullable(applicationObject.<Calendar>getPropertyValue(("jconon_application:data_ultima_modifica"))).map(x -> x.toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime()).orElse(null);
        
        //Calendar dataM = Optional.ofNullable(applicationObject.<Calendar>getPropertyValue("jconon_application:data_ultima_modifica")).orElse(applicationObject.getPropertyValue("jconon_application:data_ultima_modifica"));
        if(dataModifica !=null)
        	row.createCell(column++).setCellValue(dateFormat.format(dataModifica));
        else
