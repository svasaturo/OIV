...
define(['jquery', 'cnr/cnr', 'i18n', 'cnr/cnr.actionbutton', 'json!common', 'handlebars', 'cnr/cnr.validator', 'cnr/cnr.url', 'cnr/cnr.ui', 'cnr/cnr.criteria', 'cnr/cnr.search', 'moment', 'json!cache'], function ($, CNR, i18n, ActionButton, common, Handlebars, validator, URL, UI, Criteria, Search, moment, cache) {
  ...
  var urls = {
    call : {
      ...
      rinnovo: 'rest/manage-application/rinnovo',
      ...
      note: {
          genera: 'note',
          visualizza: 'show-note',
          firma: 'rest/call/firma-note',
          invia: 'rest/call/invia-note'
        }
    },
    application : {
      manage: 'manage-application',
      main: 'rest/manage-application/main',
      rinnovo: 'manage-application/rinnovo',
...
console.log("jconon");
	...
	console.log("dispaly 2 para "+showTitleAndDescription);
	  console.log("dispaly "+el["jconon_application:oggetto_notifica_email"]+"------ "+el.name);
...
//	if(showLastModificationDate && i18n[el.objectTypeId].localeCompare('Comunicazioni al candidato')==0){
	if(showLastModificationDate  && showTitleAndDescription==undefined && (i18n[el.objectTypeId].localeCompare('Comunicazioni al candidato')==0 || i18n[el.objectTypeId].includes('esclusione'))){
		...
		return $('<tr></tr>')
		  .append(tdText)
		  .append(tdButton);
  }else  if(showLastModificationDate == undefined && i18n[el.objectTypeId]!=undefined && i18n[el.objectTypeId].localeCompare('Comunicazioni al candidato')!=0 && !i18n[el.objectTypeId].includes('Note') && !i18n[el.objectTypeId].includes('esclusione') ) {
	  console.log("  scadenza "+el['jconon_documento_riconoscimento:data_scadenza']);
	  if( ($(".in .muted").hasClass("Curriculum") && i18n[el.objectTypeId].localeCompare('Curriculum Vitae')==0)  ||  ($(".in .muted").hasClass("Documento") && i18n[el.objectTypeId].localeCompare('Documento Riconoscimento')==0) ){
		  return;
	  }
	  var res = i18n[el.objectTypeId].split(" ");
	  var tdText,
		  tdButton,
		  scadenza,
		  isFolder = el.baseTypeId === 'cmis:folder',
		  ...
		  annotationType = $('<span class="muted annotation '+res[0]+' ">' + i18n[el.objectTypeId]  + '</span>'),
		  annotation = $('<span class="muted annotation">ultima modifica: ' + CNR.Date.format(el.lastModificationDate, null, 'DD/MM/YYYY H:mm') + '</span>');
		...
		if (showLastModificationDate === false) {
		 ...
		 if(i18n[el.objectTypeId].localeCompare('Documento Riconoscimento')==0){
			  scadenza=$('<span class="muted annotation">Scadenza: ' + CNR.Date.format(el['jconon_documento_riconoscimento:data_scadenza'], null, 'DD/MM/YYYY ') + '</span>');
			  item.after(scadenza);	
		  }
  ...
  }  else if(showLastModificationDate && showTitleAndDescription &&  i18n[el.objectTypeId].includes("Note")){
  ...
  function findAllegati(cmisObjectId, element, customType, fetchCmisObject, displayFunction, calculateTotalNumItems, parentProp) {
	 // console.log("find allegati "+contesto);
    var pagination = $('<div class="pagination pagination-centered"><ul></ul></div>'),
      ...
      criteria = new Criteria();
	    console.log("allegati dopo criteria "+allegati+" j alle "+JSON.stringify(allegati) );
		$.each( allegati, function( key, value ) {
    ...
    init({
      refreshFn: function (d) {
		  console.log("allegati "+allegati+" j alle "+JSON.stringify(allegati) );
