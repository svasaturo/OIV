...
define(['jquery', 'cnr/cnr', 'i18n', 'cnr/cnr.actionbutton', 'cnr/cnr.ui', 'cnr/cnr.jconon', 'cnr/cnr.url',
  ...
  ], function ($, CNR, i18n, ActionButton, UI, jconon, URL, Search, Criteria, Node, BulkInfo, cache, contesto) {
  "use strict";
 ...
 var modal=    UI.bigmodal(modalTitle, content, function () {
          if (!bulkinfo.validate()) {
 ...
 if(el.objectTypeId.includes("esperienza")){
	if((modal.find('.generale[data-value="true"]').hasClass("active") || !modal.find('.nonGenerale[data-value="true"]').hasClass("active")) || 
			modal.find('.dirigente_ruolo[data-value="true"]').hasClass("active")){
	 ...
	 modal.find("#dirigente_ruolo").parent().parent().css("display","block");
	...
	//	 modal.find('#dirigente_ruolo').unbind();
  ...
  return {
	...
	displayComunicazioni : function (el, refreshFn) {
		console.log("display com ");
      return jconon.defaultDisplayDocument(el, refreshFn, false,true);
    },
	
	displayNote : function (el, refreshFn) {
		console.log("display note ");
      return jconon.defaultDisplayDocument(el, refreshFn, false,true,true);
    },
...
//            });
        	  ...
        	  window.location = 'rest/application/print-immediate?nodeRef=' + applicationId;
          });
        }
        ...
        if (rs.totalNumItems === 0) {
          if (bandoInCorso) {
        	...
        	console.log("print "+rs.items[0]['cmis:versionLabel'].split('.')[1].length);
        
        	var version=parseFloat(rs.items[0]['cmis:versionLabel']);
        	if(rs.items[0]['cmis:versionLabel'].split('.')[1].length==1){
        		console.log("= 1");
        		if(rs.items[0]['cmis:versionLabel'].split('.')[1]==9){
        			version=parseFloat(rs.items[0]['cmis:versionLabel'].split('.')[0])+0.10;
          ...
          var newPrint = $('<button class="btn" data-dismiss="modal">Nuova stampa</button>'),
            ...
            btnPrimary,
            m = UI.modal('Stampa domanda', 'La stampa provvisoria sarà salvata nello storico domande come versione intermedia n ' +
              version, function () {
                  ...
                  row : function (el, refreshFn, permission) {
                    ...
                    tr.appendTo(tbody).append($('<td>').append('<b>Versione </b>')
                      .append($('<a class="label ' + labelClass + ' versionLabel">').on('click', function (event) {
						  console.log(" Js "+JSON.stringify(el));
