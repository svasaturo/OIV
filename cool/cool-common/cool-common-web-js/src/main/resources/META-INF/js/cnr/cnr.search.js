...
define(['jquery', 'cnr/cnr', 'i18n', 'cnr/cnr.url', 'cnr/cnr.searchfilter', 'cnr/cnr.criteria'], function ($, CNR, i18n, URL, SearchFilter, Criteria) {
 ...
 console.log("option ord "+options+" - "+JSON.stringify(options));
    var executeQuery,
        ...
        if (settings.elements.orderBy) {
          ...
          $.each(settings.fields, function (field, fieldId) {
            if (fieldId) {
              ...
              if (element.length !== 0) {
                $('<i></i>').addClass(!element[0].asc ? 'icon-caret-down' : 'icon-caret-up').appendTo(a);
          ...
          list.off('click').on('click', 'a', function (event) {
			  console.log("data click ord "+$(this).data()+" - "+$(this).data().asc+"  - "+JSON.stringify($(this)));
    ...
    //  settings.orderBy[0].field="jconon_application:cognome";
  if(!settings.type.includes("procedura_comparativa")){
      
    if(settings.elements.label!=undefined){  
      if(settings.elements.label.context!=undefined && settings.elements.label.context.activeElement.innerText.includes("Reset")){
    	  console.log("rset "+settings.elements.label.context.activeElement.innerText);
    	  if(settings.orderBy.length==0){
    		  settings.orderBy[0]={"field": "jconon_application:cognome", "asc":"true"};
    	  }else	if( settings.orderBy.length==1){
    		  settings.orderBy[0].field="jconon_application:cognome";
    		  settings.orderBy[0].asc=true;
		}else if( settings.orderBy.length>1){
			settings.orderBy.splice(1,settings.orderBy.length);
		}
      }
    }
