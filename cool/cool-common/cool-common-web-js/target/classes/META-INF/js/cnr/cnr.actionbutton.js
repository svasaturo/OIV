define(['jquery', 'json!common', 'cnr/cnr', 'cnr/cnr.ui', 'cnr/cnr.node', 'cnr/cnr.bulkinfo', 'cnr/cnr.ace', 'i18n', 'cnr/cnr.url', 'cnr/cnr.criteria', 'cnr/cnr.search' ], function ($, common, CNR, UI, Node, BulkInfo, Ace, i18n, URL, Criteria, Search) {
  ...
  function populateDropdown(dropdown, buttons, icons) {
    ...
    $.each(buttons, function (action, fn) {
			...
			console.log("buttons "+action);
	console.log("fn  "+fn);
	console.log("icon  "+icons+" j icon "+JSON.stringify(icons));
  ...
  function updateDocument(nodeRef, mimeType, refreshFn, maxUploadSize, contesto) {
    // inline editor requires Blob object and a compatible mimetype
	var modalTitle;
	  if(contesto.includes("note")){
		  modalTitle="Aggiorna nota";
	  }else{
		   modalTitle="Aggiorna comunicazione";
	  }
  ...
  function actionButton(obj, customPermission, customButtons, customIcons, refreshFn, nodropdown, maxUploadSize, extendClass) {
   ...
   /*   edit: function () {
        var content = $("<div></div>").addClass('modal-inner-fix'), bulkinfo,  fileInputs = [],
          afterRender = function () {
	...
	fileInputs.push(Node.inputWidget(null, "UPDATE", undefined,false, true));
    ...
    if (baseTypeId === "GROUP") {
	  ...
	  if(i18n[objectTypeId].localeCompare('Comunicazioni al candidato')==0){
		  console.log("action button comunicazioni ");
		  addActions({
		/*	select: function () {
			...
			update: function () {
			  updateDocument(nodeRef, mimeType, refreshFn, maxUploadSize, "comunicazioni");
		...
		/*	select_com: function () {
			...
			update: function () {
			  updateDocument(nodeRef, mimeType, refreshFn, maxUploadSize, "note");
    ...
    if (nodropdown) {
	 ...
	 if(i18n[objectTypeId].localeCompare('Comunicazioni al candidato')==0){
		splitButton = $('<div class="btn-group btn-prevent-fade" style="font-size: 20px; border: 1px solid gray; right: 51px; box-shadow: 1px 1px 1px gray; border-radius: 5px; padding-right: 20px; width: 139px;"></div>')
		...
		//  .append((buttons[choice] || buttons[defaultChoice] || buttons[Object.keys(buttons)[0]]).clone(true).addClass("btn btn-mini").addClass(warningClass))
			.append('<span style="padding-left: 47px; " > Azioni</span>')
			.append($('<button class="btn btn-mini dropdown-toggle" data-toggle="dropdown" style="left: 39px; bottom: 0.5px;"><span class="caret"></span></button>').addClass(warningClass));
		console.log("buttons "+buttons[defaultChoice]+" json buttons "+JSON.stringify(buttons));
	 }else{
		splitButton = $('<div class="btn-group btn-prevent-fade" style="font-size: 20px; border: 1px solid gray; box-shadow: 1px 1px 1px gray; border-radius: 5px; padding-right: 20px; width: 139px;"></div>')
		  .addClass(myClass + " " + extendClass)
		//  .append((buttons[choice] || buttons[defaultChoice] || buttons[Object.keys(buttons)[0]]).clone(true).addClass("btn btn-mini").addClass(warningClass))
			.append('<span style="padding-left: 47px; " > Azioni</span>')
			.append($('<button class="btn btn-mini dropdown-toggle" data-toggle="dropdown" style="left: 39px; bottom: 0.5px;"><span class="caret"></span></button>').addClass(warningClass));
